...
func (s *MCPStore) SaveOAuth2Session(cfg *oauth2.Config, tok *oauth2.Token) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *MCPStore) LoadOAuth2Session() (*oauth2.Config, *oauth2.Token, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	return state.Config, state.Token, nil
}
